...
const PainelAdm: React.FC = () => {
  ...
  const exportarChamadosParaCSV = (chamados: Chamado[]) => {
    const dadosCSV = chamados.map(chamado => ({
      ...
      nomeSuporte: chamado.suporte.nomeUsuario,
...
//   chamados.forEach(chamado => {
//     const month = new Date(chamado.dataAberturaChamado).getMonth();
//     const status = chamado.andamento.descAndamento;
//     if (counts[status]) {
//       counts[status][month]++;
//     }
//   });
...
const exportarFiltros = () => {
  ...
  exportToCSV(chamadosFiltrados, 'chamados_filtrados');
};
...
function exportarChamadosParaCSV(chamados: Chamado[]) {
  ...
  const dadosCSV = chamados.map(chamado => ({
    ...
    descChamado: chamado.descChamado,
    dataAberturaChamado: formatarData(chamado.dataAberturaChamado),
    idSuporte: chamado && chamado.suporte ? chamado.suporte.nomeUsuario : "",
    idUsuario: chamado && chamado.usuario ? chamado.usuario.nomeUsuario : "",
    idLab: chamado.idLab,
    idComputador: chamado.idComputador,
    resposta: chamado.resposta,
    nomeUsuario: chamado.usuario.nomeUsuario,
...
return (
    ...
    <div>
      ...
      <div className="select-container">
        ...
        <button onClick={exportarFiltros}>Exportar para Excel</button>
